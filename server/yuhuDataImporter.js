...
const db = new sqlite3.Database(dbPath, (err) => {
  if (err) {
    ...
    importYuhuProperties();
  }
});

// 雨湖区新增房源数据
const yuhuProperties = [
  // 第一批数据
  ...
  { name: '九华·金水湾', price: 4242, district: '雨湖区' }, // 取价格范围中间值
  { name: '祥霖九华新城商住楼', price: 3580, district: '雨湖区' },
  { name: '百合御都', price: 5205, district: '雨湖区' },
  { name: '华融山水苑', price: 2837, district: '雨湖区' },
  { name: '美的莲城首府', price: 4459, district: '雨湖区' },
  { name: '恒大翡翠华庭', price: 5792, district: '雨湖区' },
  { name: '城郊香樟园', price: 3870, district: '雨湖区' },
  { name: '天元广场（3室1厅户型）', price: 5238, district: '雨湖区' },
  { name: '天元广场·玺园（4室2厅户型）', price: 5405, district: '雨湖区' }, // 取价格范围中间值
  { name: '祥霖九华新城（3室2厅精装）', price: 3790, district: '雨湖区' },
  { name: '祥霖九华新城（复式毛坯）', price: 3147, district: '雨湖区' },

  // 第四批数据 - 多种类型
  { name: '心澄公寓', price: 4030, district: '雨湖区' },
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          ...
          // 所有数据插入完成
          if (count === total) {
            console.log('所有雨湖区数据插入完成，共插入', count, '条数据');
            db.close();
          }
        }
      }
