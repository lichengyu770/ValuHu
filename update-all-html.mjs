...
const updateHtmlFile = (filePath) => {
    ...
    if (content.includes('<title>')) {
        // 确保标题中包含ValuHub
        content = content.replace(/<title>(.*?)<\/title>/i, (match, title) => {
            if (!title.includes('ValuHub')) {
                title = title.replace(/智汇云|ZhiHuiYun/g, 'ValuHub');
                if (!title.includes('ValuHub')) {
                    title += ' - ValuHub';
                }
                updated = true;
    ...
    // 5. 更新logo引用（如果需要）
    // 这里只更新明显的SVG logo，不处理复杂情况
    if (content.includes('<svg') && content.includes('logo')) {
        // 简单处理，只更新明显的logo区域
        const logoPattern = /<div[^>]*class=["']logo["'][^>]*>([\s\S]*?)<\/div>/i;
        if (logoPattern.test(content)) {
            content = content.replace(logoPattern, '<a href="index.html" class="logo">\n                <img src="图片1.png" alt="ValuHub Logo">\n                ValuHub\n            </a>');
            updated = true;
        }
    }
