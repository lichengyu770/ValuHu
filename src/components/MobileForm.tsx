...
const MobileForm: React.FC<MobileFormProps> = ({
  ...
  }, [autoSave, autoSaveKey, showFeedback]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!hasErrors) {
      try {
        ...
        setErrors({ submit: error instanceof Error ? error.message : errorMessage });
  ...
  const clonedChildren = React.Children.map(children, (child) => {
    if (React.isValidElement(child)) {
      ...
      const isFormField = ['MobileInput', 'MobileSelect'].includes(
        child.type.displayName || (child.type as any).name
      );
...
interface MobileInputProps {
  ...
  type?: string;
  required?: boolean;
  inputMode?: React.HTMLAttributeReferrerPolicy;
  autoComplete?: string;
  error?: string;
  className?: string;
}
      ...
      <div className='relative'>
        ...
        {type === 'password' && (
          <button
            ...
            className='absolute inset-y-0 right-0 pr-4 pl-4 flex items-center text-gray-500 hover:text-gray-700 w-16 h-12 touch-manipulation active:scale-95 transition-all duration-200'
  ...
  return (
    <div className='space-y-2'>
      ...
      <div className='relative'>
        ...
        {/* 下拉箭头 */}
        <div className='absolute inset-y-0 right-0 flex items-center pr-4 pl-4 pointer-events-none text-gray-500 w-16 h-12' aria-hidden='true'>
          <svg
            className='h-5 w-5'
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
