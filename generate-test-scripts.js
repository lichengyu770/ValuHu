...
// 读取测试用例文件
const testCasesPath = path.join(__dirname, 'test-cases.md');
...
function parseTestCases(content) {
  ...
  // 解析注册/登录流程测试用例
  const authRegex =
    /## 1\. 注册\/登录流程测试用例[\s\S]*?(?=## 2\. 估价提交功能测试用例|$)/;
  const authMatch = content.match(authRegex);
  if (authMatch) {
    testCases.auth = parseTestCaseTable(authMatch[0]);
  }

  // 解析估价提交功能测试用例
  const valuationRegex =
    /## 2\. 估价提交功能测试用例[\s\S]*?(?=## 3\. 数据查询操作测试用例|$)/;
  const valuationMatch = content.match(valuationRegex);
  if (valuationMatch) {
    testCases.valuation = parseTestCaseTable(valuationMatch[0]);
  }

  // 解析数据查询操作测试用例
  const queryRegex =
    /## 3\. 数据查询操作测试用例[\s\S]*?(?=## 4\. 收藏功能测试用例|$)/;
  const queryMatch = content.match(queryRegex);
  if (queryMatch) {
    testCases.query = parseTestCaseTable(queryMatch[0]);
  }

  // 解析收藏功能测试用例
  const favoriteRegex =
    /## 4\. 收藏功能测试用例[\s\S]*?(?=## 5\. 测试用例管理|$)/;
  const favoriteMatch = content.match(favoriteRegex);
  if (favoriteMatch) {
    testCases.favorite = parseTestCaseTable(favoriteMatch[0]);
  }
  ...
  testCases.forEach((testCase) => {
    ...
    testSteps.forEach((step) => {
      if (step.includes('打开登录页面')) {
        ...
        const username = testData.match(/用户名：(.*?)\n/)[1];
        const password =
          testData.match(/密码：(.*?)\n|密码：(.*?)$/)[1] ||
          testData.match(/密码：(.*?)$/)[1];
        testContent +=
  ...
  testCases.forEach((testCase) => {
    ...
    testSteps.forEach((step) => {
      if (step.includes('进入估价页面')) {
        ...
        const propertyType = testData.match(/房产类型：(.*?)\n/)[1];
        const orientation = testData.match(/朝向：(.*?)\n/)[1];
  ...
  testCases.forEach((testCase) => {
    ...
    expectedResults.forEach((result) => {
      if (result.includes('收藏成功')) {
        testContent += '    cy.contains(\"已收藏\").should(\"be.visible\");\n';
...
function main() {
  ...
  fs.writeFileSync(
    path.join(__dirname, 'cypress', 'e2e', 'auth.cy.js'),
    cypressTests
  );
  ...
  fs.writeFileSync(path.join(__dirname, 'tests', 'api.test.js'), jestTests);
